...
model2 = Sequential()
model2.add(Dense(12, activation='relu', input_dim=len(filtFeat)))
...
bestEpoch, bestBatch = Hypereval(model2)
model2.fit(xte, yte, epochs=bestEpoch, batch_size=bestBatch )
yhatSubmit = np.around(model2.predict(y[filtFeat]))

scores = model2.evaluate(xte, yte)

for i in range(len(scores)):
    print("\nFinal %s: %f%%" % (model2.metrics_names[i], scores[i]))
...
df.to_csv('deep_answer2RFE.csv', index=False)
