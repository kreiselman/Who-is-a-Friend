...
model4 = Sequential()
model4.add(Dense(36, activation='relu', input_dim=len(filtFeat)))
...
bestEpoch, bestBatch = Hypereval(model4)
model4.fit(xte, yte, epochs=bestEpoch, batch_size=bestBatch )
yhatSubmit = np.around(model4.predict(y[filtFeat]))

scores = model4.evaluate(xte, yte)

for i in range(len(scores)):
    print("\nFinal %s: %f%%" % (model4.metrics_names[i], scores[i]))
...
df.to_csv('deep_answer4RFE.csv', index=False)
