...
xtr, validX, ytr, validY = train_test_split(xtr, ytr, test_size=0.5, shuffle = False)
...
model1 = Sequential()
model1.add(Dense(1, activation='sigmoid', input_dim=6))
model1.compile(optimizer='SGD',
 loss='binary_crossentropy',
 metrics=['accuracy'])

model2 = Sequential()
model2.add(Dense(12, activation='relu', input_dim=6))
...
model4 = Sequential()
model4.add(Dense(36, activation='relu', input_dim=6))
...
bestModel = np.argmax(results[:,1])
print("Best model is", bestModel + 1)
model = models[bestModel]

bestEpoch, bestBatch = Hypereval(model)
model.fit(xte, yte, epochs=bestEpoch, batch_size=bestBatch )
yhatSubmit = np.around(model.predict(y[features]))
